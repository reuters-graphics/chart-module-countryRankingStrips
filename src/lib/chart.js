  ...
  draw() {
    ...
    if (props.rugPlot) {
      ...
      if (props.rugProps.showSplitAxis && props.rugProps.splitAxis) {
        ...
        chartSVG.select('g.axis').classed('split-axis', true).lower();
      ...
      if (props.rugProps.annotation && !(props.histogram || props.densityPlot)) {
        ...
        const markerPos = Math.sqrt(2 * (rugPosition.height * 3) / Math.PI) + props.rugProps.activeRugZoom * 2;
        const markerSymbol = d3.symbol().type(d3.symbolTriangle).size(rugPosition.height * 3);

        const markerRadius = props.rugProps.showTipMarker ? Math.sqrt(2 * (rugPosition.height * 3) / Math.PI) : 0;
        ...
        const rugTouchArea = chartSVG.appendSelect('rect.touch-area')
